      ...
      <div className="grid grid-cols-2 gap-2">
        {BODY_PARTS.map((bp) => (
          <div
            ...
            className={`flex flex-col items-center gap-1 p-2 rounded-lg border border-white/10 bg-white/5 cursor-pointer transition-all duration-150 shadow-sm hover:border-blue-400 select-none text-xs ${selectedBodyParts.includes(bp.id) ? 'ring-2 ring-blue-400' : ''} ${selectedBodyParts.length >= 3 && !selectedBodyParts.includes(bp.id) ? 'opacity-50 pointer-events-none' : ''}`}
            style={{ minWidth: 0 }}
          >
